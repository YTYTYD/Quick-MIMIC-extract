...
int find_ADMISSIONS_NODE(struct ADMISSIONS_DATA *data, unsigned int HADM_ID, struct ADMISSIONS_DATA **result)
{
    while (data->HADM_ID != 0)
    {
        if (data->HADM_ID == HADM_ID)
        {
            // 找到, 返回
            *result = data;
            // printf("%d %d %lld %d\n", data->SUBJECT_ID, data->HADM_ID, data->ADMITIME, data->ADMIYEAR);
            return 0;
        }
        if (data->HADM_ID > HADM_ID)
            if (data->left == NULL)
...
void ADMISSIONS_table_read()
{
    ...
    ADMISSIONS_TABLE = (struct ADMISSIONS_DATA *)malloc(523740 * sizeof(struct ADMISSIONS_DATA));
    unsigned int data_size = 0;
    memset(ADMISSIONS_TABLE, 0, 523740 * sizeof(struct ADMISSIONS_DATA));
