...
select @DOB=DOB from [dbo].[PATIENTS] where SUBJECT_ID=@SUBJECT_ID
...
SELECT @sql_col = ISNULL(@sql_col + ',','') + QUOTENAME([NAME]) FROM [dbo].[features] GROUP BY [NAME]
 --select  @sql_col
SET @sql_str = '
...
TroponinI,	Hct,	Hgb,	PTT,	WBC,	Fibrinogen,	Platelets,	Age,	Gender,	Unit1,	Unit2,	HospAdmTime,	ICULOS
 FROM (
    SELECT a.Age,a.Gender,a.Unit1,a.Unit2,a.HospAdmTime,a.ICULOS,c.Name as ITEMNAME,CAST(b.VALUENUM AS float) AS VALUENUM from #temp a left join #CHARTEVENTS b on b.CHARTTIME>=a.begintime and b.CHARTTIME<=a.endtime
left join [dbo].[features] c on c.ITEMID=b.ITEMID 
union
SELECT a.Age,a.Gender,a.Unit1,a.Unit2,a.HospAdmTime,a.ICULOS,c.Name as ITEMNAME,CAST(b.VALUENUM AS float) AS VALUENUM from #temp a left join #LABEVENTS b on b.CHARTTIME>=a.begintime and b.CHARTTIME<=a.endtime
left join [dbo].[features] c on c.ITEMID=b.ITEMID ) p PIVOT 
