exec  p_data_be_MIMIC4 @subject_id = '10600653',@hadm_id='22409835'

ALTER PROCEDURE p_data_be_MIMIC4 (
 @hadm_id varchar(10)
...
--set @HADM_ID='165660'
 ...
 if OBJECT_ID('tempdb.dbo.#DRUG') is not null
   drop table #DRUG
--保存LABEVENTS、CHARTEVENTS数据到临时表

 SELECT  * into #LABEVENTS
 FROM [mimic4].[dbo].[MIMIC4_labevents] where  [hadm_id]=@hadm_id

 SELECT  * into #CHARTEVENTS from [mimic4].[dbo].[MIMIC4_chartevents] 
where  subject_id=@subject_id and [hadm_id]=@hadm_id

 SELECT  * into #DRUG from [mimic4].[dbo].[MIMIC4_prescriptions]
where  subject_id=@subject_id and [hadm_id]=@hadm_id
...
from #CHARTEVENTS  
 ...
 SELECT  @endtime2=max( cast( stoptime as datetime))
 ,@begintime2=min(cast(starttime as datetime))
  FROM #DRUG
  ...
  set @begintime=@begintime2

--年龄

declare @age int
select @age=anchor_age from [mimic4].[dbo].[MIMIC4_patients] where subject_id=@subject_id


--住院时间
	declare @ADMITTIME datetime,@HospAdmTime int
	SELECT @ADMITTIME=admittime FROM [mimic4].[dbo].[MIMIC4_admissions] where  [hadm_id]=@hadm_id
...
declare @Unit1 int=0
declare @Unit2 int=0
if exists(select 1 from [mimic4].[dbo].[MIMIC4_icustays] where  [hadm_id]=@hadm_id and	last_careunit ='MICU'or last_careunit='Medical Intensive Care Unit (MICU)'or last_careunit='Medical/Surgical Intensive Care Unit (MICU/SICU)')
begin
	set @Unit1=1
end 

if exists(select 1 from [mimic4].[dbo].[MIMIC4_icustays] where  [hadm_id]=@hadm_id and	last_careunit='SICU'or last_careunit='Surgical Intensive Care Unit (SICU)'or last_careunit='Medical/Surgical Intensive Care Unit (MICU/SICU)')
begin
	set @Unit2=1
end 
--Gender
declare @Gender int
select @Gender=case when gender='F' then 0 WHEN GENDER='M' THEN 1 else '' end  from [mimic4].[dbo].[MIMIC4_patients] where subject_id=@subject_id
